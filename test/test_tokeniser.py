...
class TokeniserTest(unittest.TestCase):
    ...
    def test_number(self):
        assert self.tags("1.5") == ["Number"]
        assert self.tags("1.5*^10") == ["Number"]
    ...
    def testUnset(self):
        assert self.tokens("=.") == [Token("Unset", "=.", 0)]

        # Continue like the above...
        self.assertEqual(self.tokens("= ."), [Token("Unset", "= .", 0)])
        self.assertEqual(
            self.tokens("=.5"), [Token("Set", "=", 0), Token("Number", ".5", 1)]
        )
        self.assertEqual(
            self.tokens("= .."), [Token("Set", "=", 0), Token("Repeated", "..", 2)]
        )

    def testIntRepeated(self):
        self.assertEqual(
            self.tokens("1.."), [Token("Number", "1", 0), Token("Repeated", "..", 1)]
        )
        self.assertEqual(
            self.tokens("1. ."), [Token("Number", "1.", 0), Token("Dot", ".", 3)]
        )
    ...
    def testFunction(self):
        self.assertEqual(
            self.tokens("x&"), [Token("Symbol", "x", 0), Token("Function", "&", 1)]
        )
        self.assertEqual(
            self.tokens("x\uf4a1"),
    ...
    def testBackslash(self):
        self.assertEqual(
            self.tokens("\\[Backslash]"), [Token("Backslash", "\u2216", 0)]
        )
        self.assertEqual(
            self.tokens("\\ a"),
